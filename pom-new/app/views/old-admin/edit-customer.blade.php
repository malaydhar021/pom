...
@endif
  ...
  <div class="row">
      <form class="form-horizontal" method="POST">
        @foreach($cust_info as $data)
      <!-- Text input-->
      <div class="col-sm-4">
      <div class="form-group">
        <label class="col-md-4 control-label" for="customer_name">Customer Name</label>
        <div class="col-md-8">
          <input id="customer_name" name="customer_name" type="text" placeholder="Forwarder Name" class="form-control input-md" value="{{$data->CUSTOMER_NAME}}" required>
        </div>
      </div>
      ...
      <div class="form-group">
        ...
        <div class="col-md-8">
                <input type="text" disabled class="form-control input-md" value="{{$data->CUSTOMER_EMAIL}}">
          <input id="email" maxlength="100" name="email" type="hidden" placeholder="Email ID" class="form-control input-md" value="{{$data->CUSTOMER_EMAIL}}" required>
      ...
      <div class="form-group">
        ...
        <div class="col-md-8">
            <input id="cname" name="company_name" type="text" placeholder="Company Name" value="{{$data->CUSTOMER_COMPANY_NAME}}" class="form-control input-md" required>
        </div>
      </div>
      <div class="form-group">
        <label class="col-md-4 control-label" for="Address">Address </label>
        <div class="col-md-8">
          <textarea id="Address" name="cust_address" placeholder="Address" class="form-control input-md">{{$data->CUSTOMER_ADDRESS}}</textarea>
        </div>
      </div>
      </div>
      <div class="col-sm-4">
      <div class="form-group">
        ...
        <div class="col-md-8">
          <select class="form-control input-md" name="cust_country" id="Country">
              ...
              <option value='{{$country_name->CNT_ID}}' <?php if($country_name->CNT_ID == $data->CUSTOMER_COUNTRY) echo 'selected';?> >{{$country_name->CNT_NICENAME}}</option>
              
             <!-- <option value='{{$data->CUSTOMER_COUNTRY}}'>{{$data->CNT_NICENAME}}</option>-->
              @endforeach
          </select>
        </div>
      </div>
      <div class="form-group">
        ...
        <div class="col-md-8">
          <input id="Zip" name="cust_zip" type="text" placeholder="Zip" value="{{$data->CUSTOMER_ZIP}}" class="form-control input-md">
        </div>
      </div>
      </div>
      @endforeach 
          ...
          <tbody>
            <?php $i = 0; $k=1;?>
              @foreach($menu as $value)
	              <td>
	                <div class="checkbox">
                            <input type="checkbox" name="checkboxes[]" class="checky" <?php foreach($cust_menu_items as $items) if($items->MENU_ID == $value->MENU_ID) echo 'checked' ?> data-size="mini" id="checkboxes-<?php echo $k; ?>" value="<?php echo $value->MENU_ID; ?>">
  ...
  <div class="text-right col-sm-10 pull-right">
    <div id="button1idGroup" class="btn-group" role="group" aria-label="Button Group">
      <a href="<?php echo URL::to('customer');?>"><button type="button" id="button1id" name="button1id" class="btn btn-danger btn-sm bacckky" aria-label="Left Button"><span class="glyphicon glyphicon-circle-arrow-left"></span> Back</button></a>
      
      <button type="submit" name='edit_customer' id="button2id" name="button2id" class="btn btn-success btn-sm" aria-label="Left Button">Save <span class="glyphicon glyphicon-floppy-disk"></span></button>
    </div>
      </div>
      </div>
      </div>
