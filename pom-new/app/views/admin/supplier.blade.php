...
@include('admin.includes.sidebar')      
    ...
    <form class="form-inline" role="form" method="post" action="" id="sercherformm">
        ...
        <table class="table table-bordered table-striped table-responsive">
          ...
          <?php if(!empty($supplier)) { ?>
            @foreach($supplier as $data)
              ...
              <td>{{$data->SUPPLIER_COMPANY_NAME}}</td>
              <td>{{$data->CNT_NICENAME}}</td>
              <td><?php echo date("Y-m-d",strtotime($data->CREATED_AT)); ?></td>
              <td><button class="btn-xs btn-success btn" onClick="window.location.href ='<?php echo URL::to('view-supplier'); ?>/{{$data->SUPPLIER_ID}}'">View</button>

                <button class="btn-xs btn-primary btn" onClick="calltheshoweredit(<?php echo $data->SUPPLIER_ID;?>);" >Edit</button>
                  @if($data->IS_ACTIVE == 1)
                  
                  <button class="btn-xs btn-danger btn" id="suspend" onClick="calltheshower(<?php echo $data->SUPPLIER_ID;?>);" >Suspend </button>

                  @elseif($data->IS_ACTIVE == 0)
 
                  <button class="btn-xs btn-success btn activatorbutt" id="suspend" onClick="calltheshower(<?php echo $data->SUPPLIER_ID;?>);">Active</button>
                  @endif
                </td>
            </tr>
            @endforeach
...
function calltheshower(dataurlid)
 {
   if (confirm("Are you sure to do this ?")) {
        window.location.href ='<?php echo URL::to('suspend-supplier'); ?>/'+dataurlid;
    }
    return false;
 }
...
function calltheshoweredit(dataurlid)
 {
   if (confirm("Are you sure to do this ?")) {
        window.location.href ='<?php echo URL::to('edit-supplier'); ?>/'+dataurlid;
    }
    return false;
 }
