...
class AdminController extends \BaseController {
	...
	public function showqc_agent($msg=null)
	{
		...
		if (\Request::isMethod('get'))
		{
	       ...
	       // $this->layout = \View::make('admin.customer',$data_msg);
	 ...
	 //         $data_msg['qcagent'] =\DB::table('tbl_mast_qcagent')
		// ->join('tbl_mast_country','tbl_mast_country.CNT_ID','=','tbl_mast_qcagent.QCAGENT_COUNTRY')
		// ->join('tbl_mast_app_user','tbl_mast_app_user.ACCOUNT_ID','=','tbl_mast_qcagent.ACCOUNT_ID')
		// ->join('tbl_mast_user_role_map','tbl_mast_user_role_map.USER_ID','=','tbl_mast_app_user.USER_ID')
		// ->join('tbl_mast_role','tbl_mast_role.ROLE_ID','=','tbl_mast_user_role_map.ROLE_ID')
		// ->select('tbl_mast_qcagent.*','tbl_mast_country.CNT_NICENAME')
		// ->where('tbl_mast_role.IS_SYS_ROLE','=',0)
		// ->get();
		...
		if (\Request::isMethod('post'))
		{
			...
			$email = \Input::get('email');
		       ...
		       $data_msg['qcagent']  =\DB::table('tbl_mast_qcagent')
					->join('tbl_mast_country','tbl_mast_country.CNT_ID','=','tbl_mast_qcagent.QCAGENT_COUNTRY')
					->select('tbl_mast_qcagent.*','tbl_mast_country.CNT_NICENAME')
					->where('QCAGENT_NAME', 'LIKE', '%'. $name .'%')
					->orwhere('QCAGENT_EMAIL', 'LIKE', '%'. $email .'%')
					->get();
